...
<body>
	...
	<%
		...
		// pageContext, request, session, application 전용 
		
		// 키값이 같은 경우, 생명주기가 가장 짧은(구체적인) request가 이긴다
		// 짧은 순 pageContext, reques, session, application.
	...
	<div> n1 + 10 = ${n1 + 10 } </div> <!--  EL은 { } 안에서 연산 -->
	<div> n1 - 10 = ${n1 - 10 } </div> 
	<div> n1 * 10 = ${n1 * 10 } </div> 
	<div> n1 / 10 = ${n1 / 10 } </div> <!--  EL 의 성격은  자바스크립트와 비슷하다  -->
	<div> n1 % 10 = ${n1 % 10 } </div>  
	<div> n1 mod 10 = ${n1 mod 10 } </div> <!--  % -->  
	
	<div> n1 + n2 = ${n1} + ${n2}</div>
	<div> n1 + n2 = ${n1 + n2}</div> <!--  연산은 {  } 안에서  -->
	...
	<div>이름: ${s1.getName()}</div>
	<div>나이: ${s1.age}</div> <!--  getter 메소드 이름 -->
