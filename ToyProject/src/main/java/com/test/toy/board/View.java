...
public class View extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (search != null && search.equals("y") && column.equals("content")) {
			content = content.replace(word, "<span style='background:gold;color:tomato;'>" + word + "</span>");
		}
		...
		// 댓글 목록 가져오기
		// ArrayList<CommentDTO> clist = dao.listComment(부모글번호);
		ArrayList<CommentDTO> clist = dao.listComment(seq);
		
		
		// 첨부파일 > 정보 가져오기
		if (dto.getAttach() != null &&  (dto.getAttach().toLowerCase().endsWith(".jpg")
